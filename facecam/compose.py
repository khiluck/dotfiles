...
# Числа РАЗНЫЕ и отрицательные, и на то есть причины:
#   глаз  — 0, то есть ровно контур век от MediaPipe: это естественное
#           раскрытие глаза. Поджатие до -2 подрезало глаз по радужке, а +2 и
#           дальше добавляет кожу века и превращает вырезку в заплатку.
#   губы  — контур идёт по самой кайме губ, и сверху в заливку попадает
#           тёмная полоска усов. Поджимаем — усы уходят, губы и зубы остаются.
# Дальше -3 уходить не стоит: глаз начинает пропадать целиком, особенно
# прищуренный, и на модели вместо глаза остаётся дырка.
DILATE = {"left_eye": 0, "right_eye": 0, "lips": -2}
