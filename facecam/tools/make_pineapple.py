...
def texture(size=1024, crown=128, sharp=0.9):
    ...
    # Полоса для листьев: от тёмно-зелёного у основания к светлому у кончика.
    g = np.linspace(0.0, 1.0, crown)[:, None]
    leaf = np.array([0.07, 0.24, 0.06]) +            (np.array([0.38, 0.66, 0.22]) - np.array([0.07, 0.24, 0.06])) * g
    # продольные жилки
    vein = 0.5 + 0.5 * np.cos(TAU * uu * 46.0)
    leaf = leaf[:, None, :] * (0.86 + 0.14 * vein)[None, :, None]
    img[Hb:] = leaf
